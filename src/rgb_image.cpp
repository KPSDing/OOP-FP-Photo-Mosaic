        ...
        for (int j=1; j<w-1; j++) {
            ...
            for (int ki = 0; ki < KERNELSIZE; ki++) {
                for (int kj = 0; kj < KERNELSIZE; kj++) {
                    ...
                    green += pixels[ni][nj][GREEN] * box_fliter_kernel[ki * KERNELSIZE + kj];
                    blue  += pixels[ni][nj][BLUE] * box_fliter_kernel[ki * KERNELSIZE + kj];
                }
            }
...
void RGBImage::Apply_Sobel_Gradient(){
    ...
    for (int i = 1; i < h - 1; i++) {
        for (int j = 1; j < w - 1; j++) {
            ...
            edge[i][j] = (edge_r[i][j] + edge_g[i][j] + edge_b[i][j]) / 3; // Averaging the channels for simplicity
        }
    }
